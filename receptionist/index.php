      ...
      <div class="col-auto col-md-3 col-xl-2 px-sm-2 px-0 text-primary" style="background-color: #4f2d01;">
        <div class="d-flex flex-column align-items-center align-items-sm-start px-3 pt-4 gap-3 text-white min-vh-100">
          ...
          <header class="mt-4 row">
            <div class="col">
              <h3 class="fw-light font-montserrat">Receptionist Workspace | Report</h3>
            </div>
          ...
          <!-- content -->
          <?php
          // $sql = "SELECT date_checkin, total_price FROM payment";
          $sql = "SELECT DATE_FORMAT(date_checkin, '%M') AS month, SUM(total_price) AS total FROM payment GROUP BY month WITH ROLLUP";
          // $dateresult = mysqli_query($koneksi, $sql);
          ...
          while ($row = mysqli_fetch_assoc($result)) {
            if ($row['month'] != NULL) {
              $date_checkins[] = $row['month'];
              $total_prices[] = $row['total'];
            }
            $totalall += $row['total'];
          }
          // echo "Total Revenue: Bnd $". $totalall;
          ?>
          ...
          <button  style="align-items: center;
            ...
            width: auto;" id="generate-report-1">Generate Report 1</button>
          ...
          <button  style="align-items: center;
            ...
            touch-action: manipulation;
            vertical-align: baseline;
            width: auto;" id="generate-report-1">Generate Report 2</button>
          <canvas id="myChart" style="display:none;"></canvas>
          <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
          <script>
              // Get the canvas element and set its display style to none
              var myChart = document.getElementById('myChart');
              myChart.style.display = 'none';
            
            // Wait for the page to load before executing the script
            window.onload = function() {
              // Get the button element and add an event listener that hides it when clicked
              var generateReport1Btn = document.getElementById('generate-report-1');
              generateReport1Btn.addEventListener('click', function() {
                var myChart = document.getElementById('myChart');
                myChart.style.display = 'block';
                generateReport1Btn.style.display = 'none';
              });
            }
              ...
              var chart = new Chart(ctx, {
                  type: 'bar',
                  data: {
                      labels: <?php echo json_encode($date_checkins); ?>,
                      datasets: [{
                          label: 'Monthly Revenue',
                          data: <?php echo json_encode($total_prices); ?>,
